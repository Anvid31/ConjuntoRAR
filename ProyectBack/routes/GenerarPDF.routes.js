...
router.get("/", async (req, res) => {
  try {
    ...
    doc
      .font("Helvetica-Bold")
      .fontSize(36)
      .text("Catálogo de Películas", { align: "center" });
    doc.moveDown(20);
    ...
    for (const pelicula of peliculas) {
      ...
      try {
        const response = await axios.get(pelicula.urlImagen, {
        ...
        console.error(
          `Error al agregar la imagen ${pelicula.urlImagen}:`,
          error
        );
        doc
          .fillColor("red")
          .fontSize(12)
          .text(`Error al cargar la imagen: ${pelicula.urlImagen}`);
      }
